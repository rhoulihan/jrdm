...
>
> v0.4.2 complete: three compounding ERD/import UX defects closed (PRs #107–#112). `projectToGraph` now runs dagre only on FK-connected entities and places isolated tables in a non-overlapping grid — a 6-entity / 0-relationship import no longer collapses to a single vertical column. `DiagramPane` is fully controlled (`useNodesState`/`onNodesChange`), re-seeding layout only on a new import identity, so user drags persist across unrelated store updates. The free-text Schema Owner field is replaced by a **Connect** button that calls `POST /api/schemas` (non-Oracle-maintained schemas with at least one table) and populates a dropdown; Import is gated on a schema selection. E2e regression guards added: (a) asserts ≥ 2 distinct CSS-transform x values among rendered React Flow nodes after a 6-table edgeless import; (b) simulates a real mouse drag and asserts the node transform changed and was not snapped back. All CI jobs (lint/typecheck/unit/integration/e2e) green.
>
> **v0.5 Map-to-Document modal (M.T1–M.T5, 2026-05-18):** The redesign's authoring core — drag an ERD entity onto the document canvas to open the **Map Table to Document** modal. M.T1 delivered the pure logic core (`fkEmbed.ts`, `workingCopy.ts`, `sampleDocument.ts`); M.T2–M.T4 built `FieldChecklist`, `MappingTree`, and the composed `MapToDocumentModal`; M.T5 (this PR) wired the production trigger (`application/x-jrdm-entity` drag payload on the entity header, `application/x-jrdm-entity` drop handler in `DocumentTree` → `openMapping`), mounted the modal in `AppShell`, added the e2e guard (drag entity → Select-All → `+ add node` → Map to Path → Save → `doc-row-sample` appears in Deploy dock with "ORDER_ITEMS" embedded key and "SAMPLE0000" synthetic etag), and updated docs. Replaces blind one-column-at-a-time dragging with a deliberate, FK-aware, batch-mapping step. All CI jobs green; container rebuilt and serving.
>
> **Entity context menu / trigger redesign (ER.T1–T4, 2026-05-19):** React Flow pointer-based node-drag (v0.4.2) and native HTML5 `draggable` are mutually exclusive on one gesture — the drag trigger for the Map-to-Document modal conflicted with node repositioning and had to be replaced. ER.T1 delivered the accessible `ContextMenu` component (role=menu/menuitem, aria-disabled, Esc, keyboard nav) and `canMapToDocument` pure gate. ER.T2 wired the menu into `DiagramPane` (`onNodeContextMenu` right-click + a `⋯` button in `EntityNode`), added the `hiddenEntities` store slice (Hide from canvas / Show hidden), and removed the native entity-drag from the entity header while preserving React Flow node repositioning. ER.T3 retired the per-column quick-drag (`<li draggable>`), the now-dead `application/x-jrdm-entity` + `application/x-jrdm-column` drop handlers in `DocumentTree`/`FieldNode`, and the superseded column-authoring tests (clean deletion, not hollow skips). ER.T4 added the replacement e2e (right-click AND `⋯` → menu; "Map to document…" gated aria-disabled with tooltip; New-view; modal → Select-All → +add-node → Map-to-Path → Save → doc-row-sample + SAMPLE0000 + NESTED PATH DDL; hide/show; assert no `draggable` elements; v0.4.2 node-reposition guard untouched), retitled the stale "drag column" test scenario, updated docs/README, rebuilt the container. All CI jobs green.
>
> **New duality view via modal (NV.T1–T4, 2026-05-19):** Completes the "New duality view from this table" authoring flow. NV.T1 added `canCreateNewView` (pure gate: `editingView === null`) and `resetEditor()` (clears working view without re-importing — preserves `project`/`relationships`/layout). NV.T2 enlarged the Map-to-Document modal (wider right column for the document tree) and implemented create-root mode: when `editingView === null` the modal disables `+ add node` and `− delete` (a duality view's root is always the document root — verified against `@jrdm/model` + both emitters — so path-building is meaningless at create time); Save writes the selected columns directly to the root with `_id` from the entity PK and calls `setSampleDocs([sampleDocument(...)])` for immediate Deploy-dock feedback. NV.T3 rewired "New duality view from this table" to `openMapping(entity)` (opens the modal) instead of `startNewView`, gated it on `canCreateNewView` (disabled with tooltip once a view exists), and added the "Reset view" toolbar button (`data-testid="reset-view"`, shown when `editingView !== null`, calls `resetEditor()`). NV.T4 extended the smoke e2e (modal opens via context menu; `+/-` both disabled in create-root; Save → `doc-row-123` + `SAMPLE0000` in Deploy dock; "New duality view…" disabled post-create; Reset → re-enabled; v0.4.2 node-reposition guard intact), added the lessons entry, updated README authoring-flow, rebuilt the container. All CI jobs green.
