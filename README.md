...
2. **Author** — **right-click an entity node** (or click the **⋯** button in the entity header) to open the context menu. Choose **New duality view from this table** (enabled when no view exists yet) to open the **Map Table to Document** modal in create-root mode: select columns, click **OK / Save** — the entity becomes the document root, `_id` is seeded from its primary key, and a synthetic sample document appears immediately in the Deploy dock (no Oracle deploy required). Once a root view exists, choose **Map to document…** for additional entities to embed: select columns; click `+ add node` to place the embed in the document tree (FK cardinality auto-sets "embed as array" for 1:N); click **Map to Path** to bind; **Save** commits and updates the sample. To start over, click the **Reset view** button that appears in the toolbar — it clears the working view without re-importing (the ERD canvas stays intact). Use **Inspect table** to open the column/PK/FK inspector drawer, or **Hide from canvas** to declutter the ERD (restorable via "Show hidden (N)").
...
| v0.5-nv   | New-view-via-modal (NV.T1–T4): create-root mode, complementary gates, Reset view, enlarged modal            | ✅ Done |
